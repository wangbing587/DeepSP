...
loadresult <- function(name){
  df_columnmaxmin_cv <- read.csv(sprintf('../data/input_columnMaxMin/%s/%sDeepSP_train.csv', name, name))
  df_columnmaxmin_test <- read.csv(sprintf('../data/input_columnMaxMin/%s/%sDeepSP_test.csv', name, name))

  df_columnorma_cv <- read.csv(sprintf('../data/input_columnnorma/%s/%sDeepSP_train.csv', name, name))
  df_columnorma_test <- read.csv(sprintf('../data/input_columnnorma/%s/%sDeepSP_test.csv', name, name))
  
  df_rowmaxmin_cv <- read.csv(sprintf('../data/input_rowMaxMin/%s/%sDeepSP_train.csv', name, name))
  df_rowmaxmin_test <- read.csv(sprintf('../data/input_rowMaxMin/%s/%sDeepSP_test.csv', name, name))
 
  df_rownorma_cv <- read.csv(sprintf('../data/input_rawdata/%s/%sDeepSP_train.csv', name, name))
  df_rownorma_test <- read.csv(sprintf('../data/input_rawdata/%s/%sDeepSP_test.csv', name, name))
  
  df_cv <- rbind(df_columnmaxmin_cv, df_columnorma_cv,  df_rowmaxmin_cv, df_rownorma_cv)
  df_cv$data_split <- 'Cross_validation'
  df_test <- rbind(df_columnmaxmin_test, df_columnorma_test,  df_rowmaxmin_test, df_rownorma_test)
...
write.csv(totalresult, '../output/Classification_DiffNormaDeepSP_result.csv', row.names = F, na = '')


unique(totalresult$Method)
########################Mean±SD#########################


totalresult <- read.csv('../output/Classification_DiffNormaDeepSP_result.csv', check.names = F)


datanames <- c('nikolovski2014', 'E14TG2aR', 'itzhak2017', 'hirst2018',
               'LOPITDC2019', 'orre2019', 'mulvey2021')
methodnames <- c('ColumnMaxMin', 'ColumnNorma','RowMaxMin', 'DeepSP')
...
table2excel(x=total_table,
            file='../output/Classification_DiffNormaDeepSP_stat.csv',
            sheetName='DiffNormaDeepSPresult',
            append = FALSE,
            add.rownames=FALSE, 
            fontName="Calibri",
            border=c("top", "bottom"))
...
testsign <- function(name){
  ...
  t_cv_noatt <- wilcox.test(df_noatt_cv$f1score, df_DeepSP_cv$f1score, 
                            alternative= 'less')[["p.value"]]
  t_test_noatt <- wilcox.test(df_noatt_test$f1score, df_DeepSP_cv$f1score, 
                              alternative= 'less')[["p.value"]]
  testresult <- data.frame(Data=name, DataSplit=c('Cross_validation','Test'),
                           `pvale(WithoutCNN/DeeSP)`=c(t_cv_nocnn, t_cv_noatt),
...
methodnames <- c('ColumnMaxMin', 'ColumnNorma','RowMaxMin', 'RowNorma(DeepSP)')
totalresult[which(totalresult$Method == 'DeepSP'), 'Method'] = 'RowNorma(DeepSP)'
...
ggsave(g_cv, filename = '../figure/Classification_DiffNormaDeepSP_CV.pdf',
       height = 6, width = 12)
...
ggsave(g_test, filename = '../figure/Classification_DiffNormaDeepSP_test.pdf',
       height = 6, width = 12)
...
ggsave(g, filename = '../figure/Classification_DiffNormaDeepSP.pdf',
       height = 9, width = 12)

ggsave(g, filename = '../figure/Classification_DiffNormaDeepSP.jpg',
