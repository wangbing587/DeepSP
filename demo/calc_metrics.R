...
load(sprintf('../data/input_rawdata/%s/%sknnquadloss.rda', dataname, dataname))
df$quadloss <- unlist(knnquadloss)
write.csv(df, paste0("../data/input_rawdata/", dataname, '/', dataname, methodname,".csv"),
          row.names = F, na = '')

methodname = 'svm'
load(paste0("../data/input_rawdata/", dataname, '/', dataname, methodname,".rda"))
df <- calcresult(svmparams@cmMatrices, dataname, methodname)
load(sprintf('../data/input_rawdata/%s/%ssvmquadloss.rda', dataname, dataname))
...
load(sprintf('../data/input_rawdata/%s/%snnetquadloss.rda', dataname, dataname))
...
load(sprintf('../data/input_rawdata/%s/%srfquadloss.rda', dataname, dataname))
...
load(sprintf('../data/input_rawdata/%s/%snbquadloss.rda', dataname, dataname))
...
loadresult <- function(name){
  df_KNN <- read.csv(sprintf('../data/input_rawdata/%s/%sknn.csv', name, name))
  df_SVM <- read.csv(sprintf('../data/input_rawdata/%s/%ssvm.csv', name, name))
  df_NNET <- read.csv(sprintf('../data/input_rawdata/%s/%snnet.csv', name, name))
  df_RF <- read.csv(sprintf('../data/input_rawdata/%s/%srf.csv', name, name))
  df_NB <- read.csv(sprintf('../data/input_rawdata/%s/%snb.csv', name, name))
  df_MAP <- read.csv(sprintf('../data/input_rawdata/%s/%smap.csv', name, name))
  df_MCMC <- read.csv(sprintf('../data/input_rawdata/%s/%smcmc.csv', name, name))
  df_DeepSP <- read.csv(sprintf('../data/input_rawdata/%s/%sDeepSP_test.csv', name, name))
...
write.csv(totalresult, '../output/Classification_testresult.csv', row.names = F, na = '')
...
totalresult <- read.csv('../output/Classification_testresult.csv', 
                        check.names = F)
...
table2excel(x=total_table,
            file='../output/Classification_testresult_stat.csv',
...
gF
ggsave(gF, filename = '../figure/Classification_F1-score.pdf',
       height = 6, width = 12)
...
gP <- ggplot(totalresult, aes(x = Method, y = Precision,fill=Method)) + 
  ...
  guides(fill = guide_legend(nrow = 1, byrow = TRUE))
gP
ggsave(gP, filename = '../figure/Classification_Precision.pdf',
       height = 6, width = 12)
...
gR
ggsave(gR, filename = '../figure/Classification_Recall.pdf',
       height = 6, width = 12)
...
gA
ggsave(gA, filename = '../figure/Classification_Accuracy.pdf',
       height = 6, width = 12)
...
gQ
ggsave(gQ, filename = '../figure/Classification_Quadloss.pdf',
       height = 6, width = 12)




ClfFAQ <- (gF + theme(legend.position="none") + labs(x = NULL)) / 
  (gA + theme(legend.position="none") + labs(x = NULL)) / gQ

ClfFAQ

ggsave(ClfFAQ, filename = '../figure/Classification_FAQ.pdf',
       height = 13, width = 12)
ggsave(ClfFAQ, filename = '../figure/Classification_FAQ.jpg',
       height = 13, width = 12, dpi = 400)



ClfPR <- (gP + theme(legend.position="none") + labs(x = NULL)) / gR

ClfPR

ggsave(ClfPR, filename = '../figure/Classification_PR.pdf',
       height = 9, width = 12)
ggsave(ClfPR, filename = '../figure/Classification_PR.jpg',
