...
out_result <- function(dataname){
  methodname = 'knn'
  load(paste0("../data/input_columnnorma/", dataname, '/', dataname, methodname,".rda"))
  df <- calcresult(knnparams@cmMatrices, dataname, methodname)
  load(sprintf('../data/input_columnnorma/%s/%sknnquadloss.rda', dataname, dataname))
  df$quadloss <- unlist(knnquadloss)
  write.csv(df, paste0("../data/input_columnnorma/", dataname, '/', dataname, methodname,".csv"),
            row.names = F, na = '')
  
  methodname = 'svm'
  load(paste0("../data/input_columnnorma/", dataname, '/', dataname, methodname,".rda"))
  df <- calcresult(svmparams@cmMatrices, dataname, methodname)
  load(sprintf('../data/input_columnnorma/%s/%ssvmquadloss.rda', dataname, dataname))
  ...
  load(sprintf('../data/input_columnnorma/%s/%snnetquadloss.rda', dataname, dataname))
  ...
  load(sprintf('../data/input_columnnorma/%s/%srfquadloss.rda', dataname, dataname))
  ...
  load(sprintf('../data/input_columnnorma/%s/%snbquadloss.rda', dataname, dataname))
...
loadresult <- function(name){
  df_KNN <- read.csv(sprintf('../data/input_columnnorma/%s/%sknn.csv', name, name))
  df_SVM <- read.csv(sprintf('../data/input_columnnorma/%s/%ssvm.csv', name, name))
  df_NNET <- read.csv(sprintf('../data/input_columnnorma/%s/%snnet.csv', name, name))
  df_RF <- read.csv(sprintf('../data/input_columnnorma/%s/%srf.csv', name, name))
  df_NB <- read.csv(sprintf('../data/input_columnnorma/%s/%snb.csv', name, name))
  df_MAP <- read.csv(sprintf('../data/input_columnnorma/%s/%smap.csv', name, name))
  df_MCMC <- read.csv(sprintf('../data/input_columnnorma/%s/%smcmc.csv', name, name))
  df_DeepSP <- read.csv(sprintf('../data/input_columnnorma/%s/%sDeepSP_test.csv', name, name))
  return(rbind(df_KNN, df_SVM, df_NNET, df_RF, df_NB, df_MAP, df_MCMC, df_DeepSP))
}

totalresult <- NULL
for (name in names[-length(names)]){
...
loadresult <- function(name){
  ...
  df_MCMC <- read.csv(sprintf('../data/input_columnnorma/%s/%smcmc.csv', name, name))
  df_DeepSP <- read.csv(sprintf('../data/input_columnnorma/%s/%sDeepSP_test.csv', name, name))
  return(rbind(df_KNN, df_SVM, df_NNET, df_RF, df_NB, df_MCMC, df_DeepSP))
}
...
totalresult[which(totalresult$Method == "ColumnNorma"), 'Method'] = 'DeepSP'
table(totalresult$Method)
write.csv(totalresult, '../output/Classification_columnnorma_testresult.csv', row.names = F, na = '')
...
totalresult <- read.csv('../output/Classification_columnnorma_testresult.csv', 
                        check.names = F)
...
total_table <- data.frame(Data = totalresult_mean$Data,
                          ...
                          Quadloss=paste0(round(totalresult_mean$Quadloss_mean, 3), '±', round(totalresult_sd$Quadloss_sd, 3)), check.names = F)
...
table2excel(x=total_table,
            file='../output/Classification_columnnorma_testresult_stat.csv',
...
ggsave(g, filename = '../figure/Classification_columnnorma.pdf',
       height =15, width = 12)


ggsave(g, filename = '../figure/Classification_columnnorma.jpg',
