...
calcquadloss <- function(object, name){
  print(name)
  load(sprintf('../data/input_columnMaxMin/%s/%sknn.rda', name, name))
  knnquadloss <- knnquadloss(knnparams, object)
  save(knnquadloss, file = sprintf('../data/input_columnMaxMin/%s/%sknnquadloss.rda', name, name))
  print('knn done!')
  
  load(sprintf('../data/input_columnMaxMin/%s/%ssvm.rda', name, name))
  svmquadloss <- svmquadloss(svmparams, data)
  save(svmquadloss, file = sprintf('../data/input_columnMaxMin/%s/%ssvmquadloss.rda', name, name))
  print('svm done!')
  # load(sprintf('../data/input_columnMaxMin/%s/%snb.rda', name, name))
  # nbquadloss <- nbquadloss(nbparams, object)
  # save(nbquadlosss, file = sprintf('../data/input_columnMaxMin/%s/%snbquadloss.rda', name, name))
  # print('nb done!')
  
  load(sprintf('../data/input_columnMaxMin/%s/%snnet.rda', name, name))
  nnetquadloss <- nnetquadloss(nnetparams, object)
  save(nnetquadloss, file = sprintf('../data/input_columnMaxMin/%s/%snnetquadloss.rda', name, name))
  print('nnet done!')
  
  load(sprintf('../data/input_columnMaxMin/%s/%srf.rda', name, name))
  rfquadloss <- rfquadloss(rfparams, data)
  save(rfquadloss, file = sprintf('../data/input_columnMaxMin/%s/%srfquadloss.rda', name, name))
  print('rf done!')
...
calcnbquadloss <- function(object, name){
  ...
  load(sprintf('../data/input_columnMaxMin/%s/%snb.rda', name, name))
  nbquadloss <- nbquadloss(nbparams, object)
  save(nbquadloss, file = sprintf('../data/input_columnMaxMin/%s/%snbquadloss.rda', name, name))
  print('nb done!')
}
