...
data <- read.csv('../output/LOPITDCapplyresult.csv', row.names = 1)
colnames(data)
...
k1 <- k2 <- TRUE
k1 <- data[, 'Subcellular.markers'] == "unknown"
k2 <- data[, 'Subcellular.markers'] != "unknown"
data <- rbind(data[k1,], data[k2,])
...
p0
ggsave(p0, filename = '../figure/rawPCA.pdf', height = 6, width = 8)
...
ggsave(p, filename = '../figure/DeepSPPCA.pdf', height = 6, width = 8)
